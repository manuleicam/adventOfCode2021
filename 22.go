...
type cuboidePos struct {
	x int
	y int
	z int
}

type cuboidePlace struct {
	lightState bool
	x1         int
	x2         int
	y1         int
...
func main() {
	...
	cuboids := make(map[cuboidePos]bool)

	cuboidsList := make([]cuboidePlace, 0)

	for _, val := range InputList {
		//fmt.Println(i)

		coordsLimits := strings.Split(val, ",")

		xLimits := strings.Split(coordsLimits[0], "..")
		yLimits := strings.Split(coordsLimits[1], "..")
		zLimits := strings.Split(coordsLimits[2], "..")

		xMax, _ := strconv.Atoi(xLimits[1])
		xMin, _ := strconv.Atoi(strings.Split(xLimits[0], "=")[1])
		yMax, _ := strconv.Atoi(yLimits[1])
		yMin, _ := strconv.Atoi(strings.Split(yLimits[0], "=")[1])
		zMax, _ := strconv.Atoi(zLimits[1])
		zMin, _ := strconv.Atoi(strings.Split(zLimits[0], "=")[1])

		//xCoords := makeCoords(xMin, xMax)
		//yCoords := makeCoords(yMin, yMax)
		//zCoords := makeCoords(zMin, zMax)

		comandType := strings.Split(strings.Split(xLimits[0], "=")[0], " ")[0]
		...
		part1_Day22(&cuboids, xMin, xMax, yMin, yMax, zMin, zMax, lightSignal)

		newCuboide := cuboidePlace{lightSignal, xMin, xMax, yMin, yMax, zMin, zMax}

		// Checks if cuboide intersect any other cuboide
		//newCuboide = transformCuboide(&cuboidsList, newCuboide)

		cuboidsListAux := checkIfCuboideIntersect(cuboidsList, newCuboide, lightSignal)

		cuboidsList = append(cuboidsList, cuboidsListAux...)

	}

	lightsOn := 0

	for _, cuboide := range cuboidsList {

		lightsOn += checkLightInsideCuboide(cuboide)

	}
	//fmt.Println(cuboids)

	fmt.Println("PARTE 1:", getAllCubOn(cuboids))
	fmt.Println("PARTE 2:", lightsOn)

}

func checkIfCuboideIntersect(cuboidsList []cuboidePlace, newCuboide cuboidePlace, lightSignal bool) []cuboidePlace {

	cuboidsListAux := make([]cuboidePlace, 0)

	for _, cuboide := range cuboidsList {

		intersectCuboide, intersect := getCuboideIntersect(newCuboide, cuboide, lightSignal)

		if intersect {
			cuboidsListAux = append(cuboidsListAux, intersectCuboide)
		}

	}

	if lightSignal {
		cuboidsListAux = append(cuboidsListAux, newCuboide)
	}

	return cuboidsListAux

}

func getCuboideIntersect(newCuboide cuboidePlace, cuboide cuboidePlace, lightSignal bool) (cuboidePlace, bool) {

	newX1 := getMaxCoord(cuboide.x1, newCuboide.x1)
	newX2 := getMinCoord(cuboide.x2, newCuboide.x2)
	newY1 := getMaxCoord(cuboide.y1, newCuboide.y1)
	newY2 := getMinCoord(cuboide.y2, newCuboide.y2)
	newZ1 := getMaxCoord(cuboide.z1, newCuboide.z1)
	newZ2 := getMinCoord(cuboide.z2, newCuboide.z2)

	if newX1 > newX2 || newY1 > newY2 || newZ1 > newZ2 {
		return cuboidePlace{}, false
	}

	newCuboideLight := lightSignal

	if lightSignal && cuboide.lightState {
		newCuboideLight = false
	} else if !lightSignal && !cuboide.lightState {
		newCuboideLight = true
	}

	return cuboidePlace{newCuboideLight, newX1, newX2, newY1, newY2, newZ1, newZ2}, true

}

func getMinCoord(x int, x2 int) int {
...
func getMaxCoord(x int, x2 int) int {
...
func checkLightInsideCuboide(newCuboide cuboidePlace) int {

	side1 := absCust(newCuboide.x2 - newCuboide.x1 + 1)
	side2 := absCust(newCuboide.y2 - newCuboide.y1 + 1)
	side3 := absCust(newCuboide.z2 - newCuboide.z1 + 1)
	onOFf := -1

	if newCuboide.lightState {
		onOFf = 1
	}

	return side1 * side2 * side3 * onOFf

}

func absCust(val int) int {
...
func part1_Day22(cuboids *map[cuboidePos]bool, xMin, xMax, yMin, yMax, zMin, zMax int, lightSignal bool) {
	...
	for x := xMin; x <= xMax; x++ {
		...
		for y := yMin; y <= yMax; y++ {
			...
			for z := zMin; z <= zMax; z++ {
				...
				(*cuboids)[cuboidePos{x, y, z}] = lightSignal
			}
		}
...
func getAllCubOn(cuboids map[cuboidePos]bool) int {
	cubsOn := 0
...
func makeCoords(cMin, cMax int) []int {
...
var InputList_t1 = []string{
	"on x=10..12,y=10..12,z=10..12",
	"on x=11..13,y=11..13,z=11..13",
	"off x=9..11,y=9..11,z=9..11",
	"on x=10..10,y=10..10,z=10..10"}

var InputList_p1 = []string{
...
var InputList_tp2 = []string{"on x=-5..47,y=-31..22,z=-19..33",
