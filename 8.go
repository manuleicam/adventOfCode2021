...
type clockPos struct {
	rightTop_rightBot []string
	top               string
	leftTop_middle    []string
	leftBot_bot       []string
}

func main() {

	//splitInput := strings.Split(Input_list, "|")
	//oneObs := make([]string, 0)
	intputSplited := make([]string, 0)
	outValues := make([]string, 0)
	uniqueCodes := 0

	//Input_list_single_teste
	//Input_list_teste
	for _, display := range Input_list {

		splitInput := strings.Split(display, "|")
		intputSplited = append(intputSplited, splitInput[0])
		outValues = append(outValues, splitInput[1])
		//fmt.Println(splitInput)

	}
	//fmt.Println(intputSplited)

	//fmt.Println(Input_list_single_teste)
	...
	for i, obsline := range intputSplited {
		number = 0

		var clockArrange clockPos = clockPos{make([]string, 0), "", make([]string, 0), make([]string, 0)}

		oneObs := strings.Split(obsline, " ")
		knownNumbers := make(map[int]string)

		for _, obs := range oneObs {

			nChar := countChars(obs)

			if nChar == 2 || nChar == 3 || nChar == 4 || nChar == 7 {
				knownNumbers[converNumberCharToNumber(nChar)] = obs
			}

		}

		clockArrange.setLetterPos(knownNumbers)

		fmt.Println(clockArrange)

		outValuesSplit := strings.Split(outValues[i], " ")

		for _, obs := range outValuesSplit {
			nChar := countChars(obs)

			if nChar == 2 || nChar == 3 || nChar == 4 || nChar == 7 {
				uniqueCodes++
				number = number*10 + converNumberCharToNumber(nChar)

			}

			// 2; 3; 5;
			if nChar == 5 {
				number = number*10 + clockArrange.checkWich5CharNumber(obs)
			}

			// 0;6;9
			if nChar == 6 {
				number = number*10 + clockArrange.checkWich6CharNumber(obs)
			}
			//fmt.Println(number)
			//break
		}
	...
	fmt.Println(numberTots)

}

// 2; 3; 5;
func (clockArrange *clockPos) checkWich5CharNumber(numberString string) int {

	if timesInArray(numberString, clockArrange.rightTop_rightBot) == 2 {
		return 3
	}

	if timesInArray(numberString, clockArrange.leftTop_middle) == 2 {
		return 5
	}
...
// 0;6;9
func (clockArrange *clockPos) checkWich6CharNumber(numberString string) int {

	if timesInArray(numberString, clockArrange.rightTop_rightBot) == 1 {
		return 6
	}

	if timesInArray(numberString, clockArrange.leftTop_middle) == 1 {
		return 0
	}
...
func (clockArrange *clockPos) setLetterPos(knownNumbers map[int]string) {

	clockArrange.setRightTopAndBotPos(knownNumbers[1])
	clockArrange.setTopPos(knownNumbers[1], knownNumbers[7])
	clockArrange.setLeftTopAndMidPos(knownNumbers[1], knownNumbers[4])
	clockArrange.setLeftBotAndBotPos(knownNumbers[4], knownNumbers[8])

}

func (clockArrange *clockPos) setLeftTopAndMidPos(word_1 string, word_4 string) {

	for _, char := range word_4 {

		if !subString(string(char), word_1) {

			clockArrange.leftTop_middle = append(clockArrange.leftTop_middle, string(char))
...
func (clockArrange *clockPos) setLeftBotAndBotPos(word_4 string, word_8 string) {

	for _, char := range word_8 {

		if !subString(string(char), word_4) && string(char) != clockArrange.top {

			clockArrange.leftBot_bot = append(clockArrange.leftBot_bot, string(char))
...
func (clockArrange *clockPos) setTopPos(word_1 string, word_7 string) {

	for _, char := range word_7 {

		if !subString(string(char), word_1) {
			clockArrange.top = string(char)
			break
		}
	}
...
func (clockArrange *clockPos) setRightTopAndBotPos(word_1 string) {

	for _, char := range word_1 {
		clockArrange.rightTop_rightBot = append(clockArrange.rightTop_rightBot, string(char))
	}

}

func timesInArray(word string, wordPos []string) int {

	nTimes := 0

	for _, indChar := range word {

		if existArray(string(indChar), wordPos) {
			nTimes++
		}
...
func existArray(char string, word []string) bool {
...
func subString(pattern string, word string) bool {
...
func getCommonLetter(firstWord string, secondWord string) {

}

func converNumberCharToNumber(nChar int) int {
...
var Input_list_single_teste = []string{
